...
namespace EasySMR
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double rmrb = Convert.ToDouble(numericUpDown5.Text);
            double psi = Convert.ToDouble(numericUpDown1.Text); 
            double sd = Convert.ToDouble(numericUpDown2.Text); 
            double phi = Convert.ToDouble(numericUpDown3.Text); 
            double psi2 = Convert.ToDouble(numericUpDown3.Text);
            double f4 = Convert.ToDouble(numericUpDown4.Text); 
            double pfc = 0; 
            double wfc = 0; 
            double ftc = 0;
            double maxda = 0;
            double pll = Convert.ToDouble(numericUpDown6.Text);
            double tll = Convert.ToDouble(numericUpDown7.Text);

            int rc = dataGridView3.Rows.Count - 1; 
            int rfwc = (rc * (rc - 1)) / 2; 
            ...
            double[] wsmr = new double[rfwc];
            double[] psmr = new double[dataGridView3.Rows.Count];
            double[] tsmr = new double[dataGridView3.Rows.Count];

            double[] waf = new double[rfwc];       
            double[] paf = new double[rc];       
            double[] taf = new double[rc];      
            int tsmrc = rfwc + (2*dataGridView3.Rows.Count) - 2;     
            double[] asmr = new double[tsmrc];
            ...
            {
                a[i] = (Math.PI / 180) * (Convert.ToInt32(dataGridView3.Rows[i].Cells[2].Value) - 90);     
                b[i] = (Math.PI / 180) * Convert.ToInt32(dataGridView3.Rows[i].Cells[1].Value);
            }
            ...
            for (int i = 0; i < dataGridView3.Rows.Count - 2; i++)
            {
                for (int k = i + 1; k < dataGridView3.Rows.Count - 1; k++)
                {
                    ...
                    double wdiff = Math.Abs(sd - trend);
                    if (wdiff > 180)
                        wdiff = 360 - wdiff;
                    double apdip = (180 / Math.PI) * (Math.Atan(Math.Cos((Math.PI / 180) * wdiff) * Math.Tan((Math.PI / 180) * psi)));
            ...
            orv = Math.Round(orv, 1);
            pkv = Math.Round(pkv, 1);
            tkv = Math.Round(tkv, 1);
            wkv = Math.Round(wkv, 1);
            okv = Math.Round(okv, 1);
        ...
        void export()
        {
            ...
            worksheet.Cells[3, 10] = "Slope mass vulnerability";
            worksheet.Cells[4, 10] = "Planar failure vulnerability";
            worksheet.Cells[5, 10] = "Wedge failure vulnerability";
            worksheet.Cells[6, 10] = "Topple failure vulnerability";
            worksheet.Cells[7, 10] = "Overall kinematic vulnerability";
